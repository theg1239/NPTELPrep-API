...
app.get('/courses/:courseCode', async (req, res) => {
    ...
    try {
      ...
      // Format materials in the structure expected by the frontend
      const materials = materialsResult.rows.map(material => ({
        ...
        languages: material.languages[0] === null ? [] : material.languages
      }));
      ...
      if (materials.length === 0) {
        const courseMaterials = await fetchCourseMaterials(courseCode);
        if (courseMaterials) {
          await insertCourseMaterials(course.id, courseCode, courseMaterials);
          const newMaterialsResult = await client.query(materialsQuery, [course.id]);
          materials.push(...newMaterialsResult.rows.map(material => ({
            id: material.material_id,
            title: material.title,
            type: material.type,
            weekNumber: material.week_number,
      ...
      logger.error(`Error fetching course data for ${req.params.courseCode}: ${error.message}`);
